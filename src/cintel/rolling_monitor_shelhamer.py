"""
rolling_monitor_shelhamer.py - Project script (example).
...
Terminal command to run this file from the root project folder
    ...
    uv run python -m cintel.rolling_monitor_shelhamer
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 3.25: DEFINE ROLLING MEDIAN FOR # OF ERRORS
    # ----------------------------------------------------
    # The `errors` column holds the count of errors encountered at each timestamp.
    errors_rolling_median_recipe: pl.Expr = (
        pl.col("errors").rolling_median(WINDOW_SIZE).alias("errors_rolling_median")
    )
